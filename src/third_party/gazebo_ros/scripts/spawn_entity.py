...
class SpawnEntityNode(Node):
    ...
    def run(self):
        ...
        if self.args.bond:
            self.get_logger().warn(f"USING NAMESPACE {self.args.gazebo_namespace} FOR ENTITY {self.args.entity}")
            ...
            self.get_logger().warn("ANTHONY SPECIAL EDITION")
    ...
    def _delete_entity(self):
        ...
        srv_call = self.clientDeletion.call(req)
        # while rclpy.ok():
        #     if srv_call.done():
        #         self.get_logger().info(
