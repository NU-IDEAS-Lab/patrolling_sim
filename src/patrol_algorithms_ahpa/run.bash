...
for i in {1..3}; do
    ...
    ros2 launch patrolling_sim simulation.launch.py sim:=flatland algorithm_pkg:=patrol_algorithms_ahpa algorithm_name:=AHPA agent_count:=$AGENTS use_rviz:=false headless:=true runtime:=$RUNTIME output:="$OUTPUT_FILE" attrition_times:="$ATTRITION_TIMES"
