...
class PzAgent(BasePatrolAgent):
    ...
    def getNextNode(self):
        ...
        agentLayer = bitmap[:,:,0]
        bitmap = self._minMaxNormalize(bitmap, a=0, b=255)
    ...
    def _dist(self, a, b):
        ...
        # resolution fix
        a = (a[0] / self.pg.resolution, a[1] / self.pg.resolution)
        b = (b[0] / self.pg.resolution, b[1] / self.pg.resolution)
    ...
    def _populateStateSpace(self, observe_method, agent, radius, allow_done_agents):
        ...
        for v in self.pg.graph.nodes:
            self.get_logger().info(f"Node {v} position: {self.pg.getNodePosition(v)}, agent position: {agent.position}, distance: {self._dist(self.pg.getNodePosition(v), agent.position)}, radius: {radius}")
        agents = [a for a in agentList if self._dist(a.position, agent.position) <= radius]
        for a in agentList:
            self.get_logger().info(f"Agent {a.id} position: {a.position}, agent position: {agent.position}, distance: {self._dist(a.position, agent.position)}, radius: {radius}")
        for a in agentList:
        ...
        if observe_method in ["bitmap2"]:
            ...
            def _normPosition(pos):
                x = self._minMaxNormalize(pos[0] / self.pg.resolution, a=0.0, b=self.observe_bitmap_dims[0], minimum=0.0, maximum=self.pg.widthPixels, eps=0.01)
                y = self._minMaxNormalize(pos[1] / self.pg.resolution, a=0.0, b=self.observe_bitmap_dims[1], minimum=0.0, maximum=self.pg.heightPixels, eps=0.01)
                return x, y

            # Add agents to the observation.
            for a in agents:
                pos = _normPosition(a.position)
                self.get_logger().info(f"Agent {agent.id}: Adding agent {a.id} at position {pos}")
                bitmap[int(pos[0]), int(pos[1]), self.OBSERVATION_CHANNELS.AGENT_ID] = a.id
            
            # Add vertex idleness times to the observation.
            for v in vertices:
                pos = _normPosition(self.pg.getNodePosition(v))
                self.get_logger().info(f"Agent {agent.id}: Adding vertex {v} at position {pos}")
                bitmap[int(pos[0]), int(pos[1]), self.OBSERVATION_CHANNELS.IDLENESS] = self.idleness[v]
            
            self.get_logger().info(f"Idleness values at agent {agent.id}:\n{self.idleness}")

            self.get_logger().info(f"Agent {agent.id} has {len(agents)} agents in its observation radius.")
            self.get_logger().info(f"Agent {agent.id} has {len(vertices)} vertices in its observation radius.")
